...
@Component({
  selector: 'app-golden-triangle',
  imports: [CarouselModule, PageBanner, CommonModule, BookingForm],
  templateUrl: './golden-triangle.html',
  styleUrl: './golden-triangle.scss',
})
export class GoldenTriangle {
   destination: any;
        ...
        this.destinationService.getDestinations().subscribe({
            next: (res: any[]) => {
                this.destination = res.find(
                    (d) => d.title?.toLowerCase().trim() === 'india’s golden triangle',
                );
