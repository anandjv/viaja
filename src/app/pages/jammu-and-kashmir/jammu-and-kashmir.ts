...
@Component({
  selector: 'app-jammu-and-kashmir',
  imports: [CarouselModule, PageBanner, CommonModule, BookingForm],
  templateUrl: './jammu-and-kashmir.html',
  styleUrl: './jammu-and-kashmir.scss',
})
export class JammuAndKashmir {
   destination: any;
        ...
        this.destinationService.getDestinations().subscribe({
            next: (res: any[]) => {
                this.destination = res.find(
                    (d) => d.title?.toLowerCase().trim() === 'jammu and kashmir',
                );
